...
def run():
    with sync_playwright() as p:
        ...
        # Click on '🚗' avatar
        # The avatars are in .avatar-option
        # We find text "🚗"
        page.get_by_text("🚗").click()

        # Check it is selected
        expect(page.locator(".avatar-option.selected")).to_have_text("🚗")

        page.get_by_placeholder("Nazwa Pokoju (np. DOMOWKA)").fill("EMOJI_ROOM")
        page.get_by_role("button", name="GRAJ").click()

        # Wait for Room
        expect(page.get_by_role("heading", name="POCZEKALNIA: EMOJI_ROOM")).to_be_visible()

        # Check Avatar in Lobby
        # We expect to see "🚗 EmojiPlayer"
